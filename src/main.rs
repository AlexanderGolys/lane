...
const HELP: &str = "lane compiles lane source files into GLSL.\n\nUsage:\n  lane [SOURCE [TARGET]] [--show]\n  lane SOURCE [--frag=FRAG] [--vert=VERT] [--version=VERSION] [--target=opengl|vulkan]\n  lane SOURCE [--frag-spv=SPV] [--vert-spv=SPV]\n  lane repl\n  lane preview SOURCE\n  lane list [NAME]\n  lane list 2d\n  lane list 3d\n  lane list all\n  lane -pc, --print-completion <bash|zsh|fish>\n  lane -h, --help\n\nWhen SOURCE is omitted, lane opens the interactive shell when stdin is a terminal and reads source from stdin otherwise. `lane repl` opens the same shell explicitly. The REPL keeps submitted Lane code, REPL messages, and generated GLSL in one padded colored transcript by default, with one character of inner left padding, source line numbers on submitted Lane entries, adjacent submitted Lane entries merged into one box, blank rows between boxes, and errors shown as red boxes with one blank row between command lines and error boxes; `/info` shows loaded modules, used directives, and provided objects. `/show` opens a native Vulkan preview window for the current session. `/split` toggles a split view where generated GLSL is shown only in its separate pane and toggling back restores the linear transcript. REPL commands ignore trailing spaces. Up and Down recall submitted input history across sessions. Clicking a submitted Lane entry or its generated GLSL highlights both parts of that submission. When TARGET is present, lane writes GLSL to that path instead of stdout. Use --show or -s with SOURCE TARGET to also print the compiled GLSL. Preview shader flags write complete fragment and/or vertex shaders; VERSION defaults to 300es for OpenGL/WebGL. Vulkan preview SPIR-V output and `lane preview` use glslc.";
