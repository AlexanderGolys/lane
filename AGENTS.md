...
- The command-line interface, preview runner, and REPL live in the separate `crates/lane-cli` workspace crate.
...
- Keep CLI/REPL/editor behaviour out of the compiler crate; place it in `crates/lane-cli` or `crates/lane-lsp`.
- Keep completion catalogues in editor/tooling layers such as lane-lsp; the compiler API should not expose completion data.
...
- `cargo run -p lane-cli -- test.lane`
